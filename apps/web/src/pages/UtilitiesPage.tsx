...
const EMPTY_FORM: FormState = { date: "", amount: "", serviceDays: "" };
...
export function UtilitiesPage({ onLogout, onNavigate }: Props) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (isNaN(serviceDays) || serviceDays < 1) {
      setFormError("Service days must be at least 1.");
      return;
    }
    setSubmitting(true);
    setFormError(null);
    try {
      if (editing) {
        await edit(editing.id, form.date, amount, serviceDays);
      } else {
        await add(activeType, form.date, amount, serviceDays);
      }
      closeForm();
    } catch (e) {
